...
void Serial2Mqtt::init()
{
	...
	_mqttSubscribedTo = "test/" + _serial2mqttDevice + "/#";
	_mqttProgrammerTopic = "test/" + _serial2mqttDevice + "/serial2mqtt/flash";
 ...
 * subscribe : [0,"test/myTopic","ABCD"]
 * publish : [1,"test/topic1","message1",0,0,"ABCD"]
			...
			if (json.containsKey("cmd"))
			{
				...
				if (cmd.compare("MQTT-PUB") == 0 && json.containsKey("topic") && json.containsKey("message"))
				{
					...
					if (token[1].compare(_mqttDevice) != 0)
					{
						...
						_mqttDevice = token[1];
						_mqttSubscribedTo = "test/" + _mqttDevice + "/#";
						mqttSubscribe(_mqttSubscribedTo);
					}
