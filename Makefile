...
#	git clone https://github.com/vortex314/Common $(PKG_BUILD_DIR)/Common
	$(CP) ./src/ArduinoJson $(PKG_BUILD_DIR)/ArduinoJson
	$(CP) ./src/paho.mqtt.c $(PKG_BUILD_DIR)/paho.mqtt.c
	$(CP) ./src/Common $(PKG_BUILD_DIR)/Common
	$(CP) ./src/serial2mqtt $(PKG_BUILD_DIR)/serial2mqtt
	$(Build)
endef
...
define Build/Compile
	...
	cd $(PKG_BUILD_DIR)/paho.mqtt.c && CC='$(TARGET_CC) -fhonour-copts' AR=$(TARGET_AR) ../serial2mqtt/makePaho.sh
